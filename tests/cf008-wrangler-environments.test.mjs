...
function parseJsonc(text) {
  // Strip // line comments (wrangler.jsonc uses only this form here).
  const stripped = text
    .split("\n")
    .map((line) => {
      const idx = line.indexOf("//")
      if (idx === -1) return line
      // Keep // inside strings (none expected in this file).
...
test("CF-008: POC top-level and production env remain distinct and safe", async () => {
  ...
  assert.doesNotMatch(text, /PUBLIC_SITE_PAYLOAD_URL|SUPABASE_ANON_KEY|PUBLIC_LEADS_INTAKE_URL/)
  ...
  assert.equal(prod.vars?.DEPLOY_ENV, "staging")
  ...
  assert.notEqual(prod.vars?.DEPLOY_ENV, "production")
