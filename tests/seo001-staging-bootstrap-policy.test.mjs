...
test("staging bootstrap is pinned to isolated Worker and cannot mutate routing", async () => {
  ...
  assert.match(active, /wrangler triggers deploy --env staging/)
  assert.doesNotMatch(active, /\bdns_records\b|workers\/domains\/records/i)
  assert.doesNotMatch(active, /wrangler triggers deploy --env production/)
